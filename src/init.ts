...
import { generateHookScript, HOOK_FILENAME } from './install/hook-shim.js';
import {
  installCursorProjectHooks,
  removeCursorProjectHooks,
  cursorProjectHooksAction,
  installCopilotProjectHooks,
  removeCopilotProjectHooks,
  copilotProjectHooksAction,
} from './install/project-hooks.js';
...
// ── Hook script (Claude Code global) ─────────────────────────────────────

export { generateHookScript } from './install/hook-shim.js';
...
async function installHookScript(): Promise<{ changed: boolean }> {
  ...
  const alreadyRegisteredPost = hooks.PostToolUse.some((entry: unknown) => {
  ...
  const alreadyRegisteredPre = hooks.PreToolUse.some((entry: unknown) => {
    const e = entry as { hooks?: Array<{ command?: string }> };
    return e.hooks?.some(h => h.command?.includes(HOOK_FILENAME));
  });
...
async function removeHookScript(): Promise<{ changed: boolean }> {
  ...
  for (const event of ['PostToolUse', 'PreToolUse'] as const) {
...
async function configureCursor(projectDir: string): Promise<{ changed: boolean }> {
  ...
  const rulesChanged = existing !== desired;
  if (rulesChanged) {
    await atomicWrite(target, desired);
  }

  const hooks = await installCursorProjectHooks(projectDir);
  return { changed: rulesChanged || hooks.changed };
}
...
async function removeCursor(projectDir: string): Promise<{ changed: boolean }> {
  ...
  const hooks = await removeCursorProjectHooks(projectDir);
  return { changed: changed || hooks.changed };
}

// ── VS Code Copilot (.github/copilot-instructions.md) ──────────────────

async function configureCopilot(projectDir: string): Promise<{ changed: boolean }> {
  const instructions = await upsertMarkedSection(join(projectDir, '.github', 'copilot-instructions.md'));
  const hooks = await installCopilotProjectHooks(projectDir);
  return { changed: instructions.changed || hooks.changed };
}

async function removeCopilot(projectDir: string): Promise<{ changed: boolean; backedUp: boolean }> {
  const instructions = await removeMarkedSection(join(projectDir, '.github', 'copilot-instructions.md'));
  const hooks = await removeCopilotProjectHooks(projectDir);
  return { changed: instructions.changed || hooks.changed, backedUp: instructions.backedUp };
}
...
/** Action for the Cursor rule + project hooks. */
async function cursorAction(projectDir: string): Promise<PlanAction> {
  ...
  const rulesOk = existing === buildCursorMdc();
  const hooksOk = (await cursorProjectHooksAction(projectDir)) === 'unchanged';
  return rulesOk && hooksOk ? 'unchanged' : 'update';
}

async function copilotAction(projectDir: string): Promise<PlanAction> {
  const instructions = await markedSectionAction(join(projectDir, '.github', 'copilot-instructions.md'));
  const hooks = await copilotProjectHooksAction(projectDir);
  if (instructions === 'unchanged' && hooks === 'unchanged') return 'unchanged';
  if (instructions === 'create' && hooks === 'create') return 'create';
  return 'update';
}
...
/** Action for Claude Code hooks (PostToolUse install vet + PreToolUse DIY detect). */
async function hookAction(): Promise<PlanAction> {
  ...
  const settings = await readSettingsJson();
  const hooks = settings.hooks as { PostToolUse?: unknown[]; PreToolUse?: unknown[] } | undefined;
  const registeredPost = Array.isArray(hooks?.PostToolUse) && hooks!.PostToolUse.some((entry) => {
    const e = entry as { hooks?: Array<{ command?: string }> };
    return e.hooks?.some((h) => h.command?.includes(HOOK_FILENAME));
  });
  const registeredPre = Array.isArray(hooks?.PreToolUse) && hooks!.PreToolUse.some((entry) => {
    const e = entry as { hooks?: Array<{ command?: string }> };
    return e.hooks?.some((h) => h.command?.includes(HOOK_FILENAME));
  });

  if (fileMatches && registeredPost && registeredPre) return 'unchanged';
...
async function buildInstallPlan(
  ...
  items.push({
    label: 'Claude Code · hooks (install vet + DIY detect)',
  ...
  if (opts.all || detection.cursor.detected) {
    items.push({
      label: 'Cursor · rule + hooks (DIY detect + install vet)',
  ...
  if (opts.all || detection.copilot.detected) {
    items.push({
      label: 'VS Code Copilot · instructions + hooks',
      path: join(projectDir, '.github', 'copilot-instructions.md'),
      action: await copilotAction(projectDir),
...
function printPostInstallSummary(apiKey?: string): void {
  ...
  console.log('\nHooks: Claude Code (global), Cursor + Copilot (per-project). Codex uses instructions only — no hook API yet.');
  console.log('The `starlog` CLI works in any terminal; Cursor, Copilot, and Codex get instruction files.');
